public class Transaction implements Comparable<Transaction> {
	...
	public int compareTo(Transaction other) {
	  ...
	  return (int)(this.user.getBalance() - other.getUser().getBalance());
	}
	...
	public Transaction(Account user, double amount, Type type) {
		...
		else if (this.type == Type.WITHDRAWAL)
		  this.priority = Priority.NORMAL;
		
		else {//if (this.type == Type.LOAN_APPLICATION) {
		  if (this.user.getBalance() * 3 > this.amount)
            this.priority = Priority.URGENT;
		
  		  else //(this.user.getBalance() * 3 > this.amount)
